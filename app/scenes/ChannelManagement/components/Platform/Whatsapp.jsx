...
class Whatsapp extends Component {
  ...
  async submit(e) {
    ...
    try {
      await this.props.update({ whatsapp: this.state.payload });
      this.props.onClose();
    } catch (err) {
      throw err;
    }
    ...
    return (
      ...
      >
        <form onSubmit={this.submit}>
          <b>Link Form</b>
          <p className="fg-shade m-t-20">Whatsapp Account Data</p>
          <Input
            onChange={this.onChange}
            label={formattedMessageHelper('platform.whatsapp.apiKey')}
          ...
          <Input
            onChange={this.onChange}
            label={formattedMessageHelper('platform.whatsapp.phoneNumber')}
            styles="form-group-default"
            base={{
              name: 'phoneNumber',
              value: this.state.payload.phoneNumber,
              disabled: isFetching,
            }}
            required
          />
          <Input
            onChange={this.onChange}
            label={formattedMessageHelper('platform.whatsapp.webhook')}
